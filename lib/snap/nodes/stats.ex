defmodule Snap.Nodes.Stats do
  ...
  def stats(cluster, filters \\ [], opts \\ []) do
    nodes = parse_nodes(filters)
    stats = parse_stats(filters)
    index_metrics = parse_index_metrics(filters)
  ...
  defp parse_index_metrics([index_metrics: index_metrics]) when is_binary(index_metrics), do: index_metrics
